...
namespace FullModdedFuriesAPI.Installer.Framework
{
    /// <summary>Manages paths for the FMODF installer.</summary>
    internal class InstallerPaths
    {
        ...
        ****/
        ...
        /// <summary>The full path to FMODF's internal configuration file.</summary>
        ...
        /// <summary>The full path to the installed FMODF launcher on Linux/macOS before it's renamed.</summary>
        public string UnixFmodfLauncherPath { get; }

        /// <summary>The full path to the vanilla game launcher on Linux/macOS after FMODF is installed.</summary>
        ...
        *********/
        ...
        public InstallerPaths(DirectoryInfo bundleDir, DirectoryInfo gameDir, string gameExecutableName)
        {
            ...
            this.BundleApiUserConfigPath = Path.Combine(bundleDir.FullName, "fmodf-internal", "config.user.json");

            this.ExecutablePath = Path.Combine(gameDir.FullName, gameExecutableName);
            this.UnixLauncherPath = Path.Combine(gameDir.FullName, "Brawler2D");
            this.UnixFmodfLauncherPath = Path.Combine(gameDir.FullName, "FullModdedFuriesAPI");
            this.UnixBackupLauncherPath = Path.Combine(gameDir.FullName, "Brawler2D-original");
            this.ApiConfigPath = Path.Combine(gameDir.FullName, "fmodf-internal", "config.json");
            this.ApiUserConfigPath = Path.Combine(gameDir.FullName, "fmodf-internal", "config.user.json");
        }
