...
# Modified for FMODF by various contributors
...
if [ "$UNAME" == "Darwin" ]; then
    ...
    # fix "DllNotFoundException: libgdiplus.dylib" errors when loading images in FMODF
    ...
    if [ ! -xFullModdedFuriesAPI.bin.osx ] || ! cmp StardewValley.bin.osxFullModdedFuriesAPI.bin.osx >/dev/null 2>&1; then
        cp -p StardewValley.bin.osxFullModdedFuriesAPI.bin.osx
    fi
    ...
    if [ ! -t 1 ]; then # https://stackoverflow.com/q/911168/262123
        ...
        if [ "$SKIP_TERMINAL" == "false" ]; then
            ...
            echo "\"$0\" $@ --no-reopen-terminal" > /tmp/open-fmodf-terminal.sh
            chmod +x /tmp/open-fmodf-terminal.sh
            cat /tmp/open-fmodf-terminal.sh
            open -W -a Terminal /tmp/open-fmodf-terminal.sh
            rm /tmp/open-fmodf-terminal.sh
            exit 0
        fi
    fi

    # launch FMODF
    LC_ALL="C" ./FullModdedFuriesAPI.bin.osx "$@"
else
    # choose binary file to launch
    LAUNCH_FILE=""
    if [ "$ARCH" == "x86_64" ]; then
        ln -sf mcs.bin.x86_64 mcs
        cp StardewValley.bin.x86_64FullModdedFuriesAPI.bin.x86_64
        LAUNCH_FILE="./FullModdedFuriesAPI.bin.x86_64"
    else
        ln -sf mcs.bin.x86 mcs
        cp StardewValley.bin.x86FullModdedFuriesAPI.bin.x86
        LAUNCH_FILE="./FullModdedFuriesAPI.bin.x86"
    fi
    export LAUNCH_FILE
    ...
    ## terminal isn't executable; fallback to current shell or no terminal
        ...
        echo "The '$TERMINAL_NAME' terminal isn't executable. FMODF might be running in a sandbox or the system might be misconfigured? Falling back to current shell."
        env TERM=xterm LC_ALL="C" $LAUNCH_FILE "$@"
        if [ $? -eq 127 ]; then
            exec LC_ALL="C" $LAUNCH_FILE --no-terminal "$@"
        fi
