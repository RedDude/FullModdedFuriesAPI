    ...
    internal class GameContentManager : BaseContentManager
    {
        ...
        *********/
        ...
        public override T Load<T>(string assetName, LocalizedContentManager.LanguageCode language, bool useCache)
        {
            ...
            language = LanguageCode.en; //Keep english since FMF doesn't load asset language based
        ...
        *********/
        ...
        private IAssetData ApplyLoader<T>(IAssetInfo info)
        {
            ...
            if (loaders.Length > 1)
            {
                ...
                this.Monitor.Log($"Multiple mods want to provide the '{info.AssetName}' asset ({string.Join(", ", loaderNames)}), but an asset can't be loaded multiple times. FMODF will use the default asset instead; uninstall one of the mods to fix this. (Message for modders: you should usually use {typeof(IAssetEditor)} instead to avoid conflicts.)", LogLevel.Warn);
                return null;
            }
            ...
            {
                mod.LogAsMod($"Mod crashed when loading asset '{info.AssetName}'. FMODF will use the default asset instead. Error details:\n{ex.GetLogSummary()}", LogLevel.Error);
                return null;
            }
        ...
        private bool TryValidateLoadedAsset<T>(IAssetInfo info, T data, IModMetadata mod)
        {
            ...
            if (data == null)
            {
                mod.LogAsMod($"FMODF blocked asset replacement for '{info.AssetName}': mod incorrectly set asset to a null value.", LogLevel.Error);
                return false;
            }
            ...
            //             if (isFarmMap)
            //             {
            //                 mod.LogAsMod($"FMODF blocked '{info.AssetName}' map load: {reason}", LogLevel.Error);
            //                 return false;
            //             }
            //             mod.LogAsMod($"FMODF found an issue with '{info.AssetName}' map load: {reason}", LogLevel.Warn);
            //         }
            //     }
            // }
