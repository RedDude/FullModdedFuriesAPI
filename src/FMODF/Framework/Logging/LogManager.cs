...
using FullModdedFuriesAPI.Framework.Commands;
...
namespace FullModdedFuriesAPI.Framework.Logging
{
    ...
    internal class LogManager : IDisposable
    {
        ...
        private readonly ReplaceLogPattern[] ReplaceConsolePatterns =
        {
            ...
            new ReplaceLogPattern(
                ...
                replacement:
                    ...
                    "Oops! Steam achievements won't work because Steam isn't loaded. See 'Launch FMODF through Steam or GOG Galaxy' in the install guide for more info: https://fmodf.io/install.",
#else
        ...
        ****/
        ...
        /// <param name="colorConfig">The colors to use for text written to the FMODF console.</param>
        ...
        public LogManager(string logPath, ColorSchemeConfig colorConfig, bool writeToConsole, bool isVerbose, bool isDeveloperMode, Func<int?> getScreenIdForLog)
        {
            ...
            this.Monitor = this.GetMonitor("FMODF");
        ...
        /// <summary>Get a monitor instance derived from FMODF's current settings.</summary>
        ...
        /// <summary>Set the title of the FMODF console window.</summary>
        ...
        ****/
        ...
        public void WriteCrashLog()
        {
            ...
            {
                this.Monitor.Log($"FMODF failed trying to track the crash details: {ex.GetLogSummary()}", LogLevel.Error);
            }
        }
        ...
        /// <summary>Check whether FMODF crashed or detected an update during the last session, and display them in the FMODF console.</summary>
        public void HandleMarkerFiles()
        {
            ...
            if (File.Exists(Constants.UpdateMarker))
            {
                ...
                if (SemanticVersion.TryParse(rawUpdateFound[0], out ISemanticVersion updateFound))
                {
                    if (Constants.ApiVersion.IsPrerelease() && updateFound.IsNewerThan(Constants.ApiVersion))
                    {
                        ...
                        this.Monitor.Log("A new version of FMODF was detected last time you played.", LogLevel.Error);
                        this.Monitor.Log($"You can update to {updateFound}: {url}.", LogLevel.Error);
                        this.Monitor.Log("Press any key to continue playing anyway. (This only appears when using a FMODF beta.)", LogLevel.Info);
                        Console.ReadKey();
                    }
                }
            ...
            {
                this.Monitor.Log("The game crashed last time you played. If it happens repeatedly, see 'get help' on https://fmodf.io.", LogLevel.Error);
                this.Monitor.Log("If you ask for help, make sure to share your FMODF log: https://fmodf.io/log.", LogLevel.Error);
        ...
        /// <summary>Log a fatal exception which prevents FMODF from launching.</summary>
        ...
        ** General log output
        ****/
        /// <summary>Log the initial header with general FMODF and system details.</summary>
        /// <param name="modsPath">The path from which mods will be loaded.</param>
        /// <param name="customSettings">The custom FMODF settings.</param>
        public void LogIntro(string modsPath, IDictionary<string, object> customSettings)
        {
            // log platform & patches
            {
                // this.Monitor.Log($"FMODF {Constants.ApiVersion} with Full Metal Furies {Constants.GameVersion} on {EnvironmentUtility.GetFriendlyPlatformName(Constants.Platform)}", LogLevel.Info);
                this.Monitor.Log($"FMODF {Constants.ApiVersion} with Full Metal Furies FIX on {EnvironmentUtility.GetFriendlyPlatformName(Constants.Platform)}", LogLevel.Info);
        ...
        public void LogSettingsHeader(SConfig settings)
        {
            ...
            if (settings.DeveloperMode)
                this.Monitor.Log("You enabled developer mode, so the console will be much more verbose. You can disable it by installing the non-developer version of FMODF.", LogLevel.Info);

            // warnings
            if (!settings.CheckForUpdates)
                this.Monitor.Log("You disabled update checks, so you won't be notified of new FMODF or mod updates. Running an old version of FMODF is not recommended. You can undo this by reinstalling FMODF.", LogLevel.Warn);
            if (!settings.RewriteMods)
                this.Monitor.Log("You disabled rewriting broken mods, so many older mods may fail to load. You can undo this by reinstalling FMODF.", LogLevel.Info);
        ...
        *********/
        ...
        /// <summary>Get human-readable labels to log for detected FMODF and Full Metal Furies customizations.</summary>
        ...
        private void LogModWarnings(IEnumerable<IModMetadata> mods, IModMetadata[] skippedMods, bool logParanoidWarnings)
        {
            ...
            if (modsWithWarnings.Any())
            {
                ...
                this.LogModWarningGroup(modsWithWarnings, ModWarning.BrokenCodeLoaded, LogLevel.Error, "Broken mods",
                    "These mods have broken code, but you configured FMODF to load them anyway. This may cause bugs,",
                ...
                this.LogModWarningGroup(modsWithWarnings, ModWarning.UsesUnvalidatedUpdateTick, LogLevel.Info, "Bypassed safety checks",
                    "These mods bypass FMODF's normal safety checks, so they're more likely to cause errors or save",
                ...
                if (logParanoidWarnings)
                {
                    this.LogModWarningGroup(
                        ...
                        blurb: new[]
                        {
                            ...
                            "FMODF console. (This is usually legitimate and innocent usage; this warning is only useful for",
                ...
                this.LogModWarningGroup(modsWithWarnings, ModWarning.NoUpdateKeys, LogLevel.Debug, "No update keys",
                    "These mods have no update keys in their manifest. FMODF may not notify you about updates for these",
