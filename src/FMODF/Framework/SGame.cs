...
// using FullModdedFuriesAPI.Framework.Input;
...
{
    /// <summary>FMODF's extension of the game's core <see cref="GameController"/>, used to inject events.</summary>
    internal class SGame : GameController
    {
        /*********
        ** Fields
        *********/
        /// <summary>Encapsulates monitoring and logging for FMODF.</summary>
        private readonly Monitor Monitor;

        /// <summary>Manages FMODF events for mods.</summary>
        private readonly EventManager Events;

        /// <summary>The maximum number of consecutive attempts FMODF should make to recover from a draw error.</summary>
        ...
        private readonly Action<GameController, GameTime, Action> OnUpdating;
        ...
        *********/
        ...
        /// <summary>The number of ticks until FMODF should notify mods that the game has loaded.</summary>
        ...
        /// <summary>Whether the game is saving and FMODF has already raised <see cref="IGameLoopEvents.Saving"/>.</summary>
        public bool IsBetweenSaveEvents { get; set; }

        /// <summary>Whether the game is creating the save file and FMODF has already raised <see cref="IGameLoopEvents.SaveCreating"/>.</summary>
        ...
        *********/
        ...
        /// <param name="monitor">Encapsulates monitoring and logging for FMODF.</param>
        /// <param name="reflection">Simplifies access to private game code.</param>
        /// <param name="eventManager">Manages FMODF events for mods.</param>
        ...
        public SGame(
            ...
            Action<GameController, GameTime, Action> onUpdating
        )
        {
            this.m_gameArgs ??= new string[] { };
            // init XNA
            ...
            eventManager.ConfigLoaded.RaiseEmpty();
        }
        ...
        //     if (input == null)
        //         throw new InvalidOperationException("FMODF's input state is not in a ready state yet.");
        ...
        *********/
        ...
        // protected override LocalizedContentManager CreateContentManager(IServiceProvider serviceProvider, string rootDirectory) //WEIRD override was not a problem before
        ...
        protected override void Initialize()
        {
            ...
            this.disposableContent = CreateContentManagerImpl(this.Services, this.Content.RootDirectory);
            this.nonDisposableContent = CreateContentManagerImpl(this.Services, this.Content.RootDirectory);

            base.Initialize();

            // The game resets public static fields after the class is constructed (see
            // GameRunner.SetInstanceDefaults), so FMODF needs to re-override them here.
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (this.IsFirstTick)
            {
                ...
                this.Watchers = new WatcherCore(null); //this.InputManager);
            }
        ...
        {
            // this.Reflection.GetField<bool>(this, "m_drawCalled").SetValue(true);
            ...
            catch (Exception ex)
            {
                ...
                {
                    this.ExitGameImmediately("The game crashed when drawing, and FMODF was unable to recover the game.");
                    return;
                }
        ...
        /// <summary>Replicate the game's draw logic with some changes for FMODF.</summary>
        /// <param name="gameTime">A snapshot of the game timing state.</param>
        /// <param name="target_screen">The render target, if any.</param>
        ...
        [SuppressMessage("FMODF.CommonErrors", "AvoidNetField", Justification = "copied from game code as-is")]
        [SuppressMessage("FMODF.CommonErrors", "AvoidImplicitNetFieldCast", Justification = "copied from game code as-is")]
   ...
   // /// <summary>The method called to draw everything to the screen.</summary>
        ...
        //             catch (Exception ex)
        //             {
        //                 this.Monitor.Log($"The {activeClickableMenu.GetType().FullName} menu crashed while drawing itself during save. FMODF will force it to exit to avoid crashing the game.\n{ex.GetLogSummary()}", LogLevel.Error);
        ...
        //         catch (Exception ex)
        //         {
        //             this.Monitor.Log($"The {curMenu.GetMenuChainLabel()} menu crashed while drawing itself. FMODF will force it to exit to avoid crashing the game.\n{ex.GetLogSummary()}", LogLevel.Error);
