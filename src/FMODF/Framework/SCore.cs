...
using FullModdedFuriesAPI.Framework.Content;
using FullModdedFuriesAPI.Framework.Events;
using FullModdedFuriesAPI.Framework.Exceptions;
using FullModdedFuriesAPI.Framework.Logging;
using FullModdedFuriesAPI.Framework.Models;
using FullModdedFuriesAPI.Framework.ModHelpers;
using FullModdedFuriesAPI.Framework.ModLoading;
using FullModdedFuriesAPI.Framework.Reflection;
using FullModdedFuriesAPI.Framework.Serialization;
using FullModdedFuriesAPI.Framework.StateTracking.Comparers;
using FullModdedFuriesAPI.Framework.Utilities;
using FullModdedFuriesAPI.Internal;
...
using FullModdedFuriesAPI.Framework.StateTracking.Snapshots;
...
{
    /// <summary>The core class which initializes and manages FMODF.</summary>
    internal class SCore : IDisposable
    {
        ...
        ****/
        ...
        /// <summary>Manages the FMODF console window and log file.</summary>
        private readonly LogManager LogManager;

        /// <summary>The core logger and monitor for FMODF.</summary>
        private Monitor Monitor => this.LogManager.Monitor;

        /// <summary>Simplifies access to private game code.</summary>
        private readonly Reflector Reflection = new Reflector();

        /// <summary>Encapsulates access to FMODF core translations.</summary>
        private readonly Translator Translator = new Translator();

        /// <summary>The FMODF configuration settings.</summary>
        ...
        ****/
        ...
        /// <summary>FMODF's content manager.</summary>
        ...
        /// <summary>Manages FMODF events for mods.</summary>
        ...
        ****/
        ...
        /// <summary>The maximum number of consecutive attempts FMODF should make to recover from an update error.</summary>
        ...
        *********/
        ...
        public SCore(string modsPath, bool writeToConsole)
        {
            ...
            this.LogManager = new LogManager(logPath: logPath, colorConfig: this.Settings.ConsoleColors, writeToConsole: writeToConsole, isVerbose: this.Settings.VerboseLogging, isDeveloperMode: this.Settings.DeveloperMode, null); //getScreenIdForLog: this.GetScreenIdForLog);
            ...
            {
                this.Monitor.Log("Oops! You're running Windows, but this version of FMODF is for Linux or macOS. Please reinstall FMODF to fix this.", LogLevel.Error);
                this.LogManager.PressAnyKeyToExit();
            }
#else
            if (Constants.Platform == Platform.Windows)
            {
                this.Monitor.Log($"Oops! You're running {Constants.Platform}, but this version of FMODF is for Windows. Please reinstall FMODF to fix this.", LogLevel.Error);
                this.LogManager.PressAnyKeyToExit();
            }
#endif
        }

        /// <summary>Launch FMODF.</summary>
        /// <param name="strings"></param>
        [HandleProcessCorruptedStateExceptions, SecurityCritical] // let try..catch handle corrupted state exceptions
        public void RunInteractively(string[] args)
        {
            // initialize FMODF
            try
            {
                ...
                HarmonyPatcher.Apply("FMODF", this.Monitor,
                    SaveManagerPatcher.PatchMe(this.EventManager),
                    new LocaleBuilderPatcher(),
                    new PlayerEvPatcher(),
                    SpriteObjPatcher.PatchMe(this.EventManager),
                    new LevelParserPatcher()
                    // new CharacterPatch(),
                    // new CharacterSelectionPatcher()
                    // ScreenManagerPatcher.PatchMe(this.EventManager)
            ...
            {
                this.Monitor.Log($"FMODF failed to initialize: {ex.GetLogSummary()}", LogLevel.Error);
                this.LogManager.PressAnyKeyToExit();
                return;
            }
            ...
            try
            {
                ...
                // Brawler2D.Program.releaseBuild = true; // game's debug logic interferes with FMODF opening the game window
                // this.Game.Run();
                this.StartGame(args);
            }
            catch (Exception ex)
            ...
            finally
            {
                ...
                {
                    this.Monitor.Log($"The game ended, but FMODF wasn't able to dispose correctly. Technical details: {ex}", LogLevel.Error);
                }
            }
        }
        ...
        *********/
        ...
        private void InitializeBeforeFirstAssetLoaded()
        {
            ...
            {
                this.Monitor.Log("FMODF shutting down: aborting initialization.", LogLevel.Warn);
                return;
            }
        ...
        private void OnGameInitialized()
        {
            ...
            // this.Monitor.Log("FMODF found problems in your game's content files which are likely to cause errors or crashes. Consider uninstalling XNB mods or reinstalling the game.", LogLevel.Error);
        ...
        private void OnGameUpdating(GameController gameController, GameTime gameTime, Action runGameUpdate)
        {
            try
            {
                /*********
                ...
                *********/
                ...
                if (gameController.gameLoaded && !this.IsGameLoaded)
                {
                    this.UpdateWindowTitles();
                ...
                ** Special cases
                *********/
                // Abort if FMODF is exiting.
                if (this.CancellationToken.IsCancellationRequested)
                {
                    this.Monitor.Log("FMODF shutting down: aborting update.");
                    return;
                }
                ...
                *********/
                ...
                this.OnPlayerInstanceUpdating((SGame) gameController, gameTime, runGameUpdate);
            ...
            catch (Exception ex)
            {
                ...
                if (!this.UpdateCrashTimer.Decrement())
                    this.ExitGameImmediately("The game crashed when updating, and FMODF was unable to recover the game.");
            }
            finally
            {
                SCore.TicksElapsed++;
        ...
        private void OnPlayerInstanceUpdating(SGame instance, GameTime gameTime, Action runUpdate)
        {
            ...
            try
            {
                ...
                *********/
                ...
                // SInputState inputState = instance.InputManager;
                InputManager inputState = instance.InputManager;
                ...
                *********/
                ...
                //
                // Therefore we can just run Game1.Update here without raising any FMODF events. There's
                ...
                *********/
                ...
                if (this.Game.IsActive)
                {
                    ...
                    //         if (!isChatInput)
                    //         {
                    var cursor = instance.InputManager.GetMouseState();
                    ...
                    var inputMap = SGame.g_game.InputMap;
                ...
                {
                    bool raiseWorldEvents = true; //!state.SaveID.IsChanged; // don't report changes from unloaded => loaded
                    ...
                    if (raiseWorldEvents)
                    {
                        ...
                        PlayerObj player = playerState.Player;

                        if (playerState.ClassObj.IsChanged)
                        {
                            // if (this.Monitor.IsVerbose)
                ...
                //     /*********
                    ...
                    if (instance.IsFirstTick && !Context.IsGameLaunched)
                    {
                        ...
                        //      instance.ScreenManager.LoadScreen("Camp", 0);
                    }
        ...
        // {
        //     // get last FMODF version used with this save
        //     const string migrationKey = "Pathoschild.FMODF/api-version";
        ...
        //     // fix bundle corruption in FMODF 3.8.0
        //     // For non-English players who created a new save in FMODF 3.8.0, bundle data was
        ...
        // {
        //     this.Multiplayer.Disconnect(Brawler2D.Multiplayer.DisconnectType.ClosedGame);
        //     this.Dispose();
        // }
        ...
        private LocalizedContentManager CreateContentManager(IServiceProvider serviceProvider, string rootDirectory)
        {
            ...
            if (this.ContentCore == null)
            {
                ...
                return this.ContentCore.CreateGameContentManager("_temporaryContent");
            }
        ...
        {
            return GameController.g_game as SGame
                   ?? throw new InvalidOperationException("The current game instance wasn't created by FMODF.");
        }
        ...
        {
            string fmodfVersion = $"{Constants.ApiVersion}";

            string consoleTitle = $"FMODF {fmodfVersion} - running Full Metal Furies {Constants.GameVersion}";
            string gameTitle = $"Full Metal Furies {Constants.GameVersion} - running FMODF {fmodfVersion}";
        ...
        #region CheckForSoftwareConflicts
...
//                     this.Monitor.Log($"Found {string.Join(" and ", installedNames)} installed, which may conflict with FMODF. If you experience errors or crashes, try disabling that software or adding an exception for FMODF and Full Metal Furies.", LogLevel.Warn);
//                 else
...
//         /// <summary>Asynchronously check for a new version of FMODF and any installed mods, and print alerts to the console if an update is available.</summary>
...
//                     {
//                         // fetch update check
//                         ModEntryModel response = client.GetModInfo(new[] { new ModSearchEntryModel("Pathoschild.FMODF", Constants.ApiVersion, new[] { $"GitHub:{this.Settings.GitHubProjectName}" }) }, apiVersion: Constants.ApiVersion, gameVersion: Constants.GameVersion, platform: Constants.Platform).Single().Value;
//                         updateFound = response.SuggestedUpdate?.Version;
//                         updateUrl = response.SuggestedUpdate?.Url ?? Constants.HomePageUrl;

//                         // log message
//                         if (updateFound != null)
//                             this.Monitor.Log($"You can update FMODF to {updateFound}: {updateUrl}", LogLevel.Alert);
//                         else
//                             this.Monitor.Log("   FMODF okay.");

//                         // show errors
//                         if (response.Errors.Any())
//                         {
//                             this.Monitor.Log("Couldn't check for a new version of FMODF. This won't affect your game, but you may not be notified of new versions if this keeps happening.", LogLevel.Warn);
//                             this.Monitor.Log($"Error: {string.Join("\n", response.Errors)}");
//                         }
//                     }
//                     catch (Exception ex)
//                     {
//                         this.Monitor.Log("Couldn't check for a new version of FMODF. This won't affect your game, but you won't be notified of new versions if this keeps happening.", LogLevel.Warn);
...
//         }
        ...
        /// <param name="modDatabase">Handles access to FMODF's internal mod metadata list.</param>
        ...
        private bool TryLoadMod(IModMetadata mod, IModMetadata[] mods, AssemblyLoader assemblyLoader, InterfaceProxyFactory proxyFactory, JsonHelper jsonHelper, ContentCoordinator contentCore, ModDatabase modDatabase, HashSet<string> suppressUpdateChecks, out ModFailReason? failReason, out string errorReasonPhrase, out string errorDetails)
        {
            ...
            else
            {
                ...
                {
                    string[] updateUrls = new[] {modDatabase.GetModPageUrlFor(manifest.UniqueID), "https://fmodf.io/mods"}.Where(p => p != null).ToArray();
                ...
                try
                {
                    ...
                    {
                        if (!this.ModRegistry.AreAllModsLoaded)
                            throw new InvalidOperationException("Can't access content packs before FMODF finishes loading mods.");
                    ...
                    IModHelper modHelper;
                    {
                        ...
                        IMultiplayerHelper multiplayerHelper = null; //new MultiplayerHelper(manifest.UniqueID, this.Multiplayer);
                        ILocateBuilderHelper locateBuilderHelper = new LocateBuilderHelper(manifest.UniqueID);
                        IDatabaseHelper databaseHelper = new DatabaseHelper(manifest.UniqueID);

                        modHelper = new ModHelper(manifest.UniqueID, mod.DirectoryPath,
                            () => null,
                            events,
                            contentHelper,
                            contentPackHelper,
        ...
        private void ReloadTranslations(IEnumerable<IModMetadata> mods)
        {
            // core FMODF translations
            {
                ...
                {
                    this.Monitor.Log("FMODF couldn't load some core translations. You may need to reinstall FMODF.", LogLevel.Warn);
        ...
        /// <summary>Get the map display device which applies FMODF features like tile rotation to loaded maps.</summary>
        ...
        /// <summary>Delete normal (non-crash) log files created by FMODF.</summary>
        ...
        private void StartGame(string[] args)
        {
            ...
            if (GlobalEV.CREATE_RETAIL_VERSION || GlobalEV.CREATE_DEMO_VERSION || GlobalEV.CREATE_CONSOLE_QA_VERSION)
            {
                GlobalEV.PRELOAD_ALL_DATA = false;
                GlobalEV.ENABLE_ONLINE_CHEAT = false;
                GlobalEV.DEBUG_MODE = false;
                GlobalEV.UNLOCK_ALL_SKILLS = false;
                GlobalEV.INFINITE_PURCHASE_POWER = false;
                GlobalEV.RANDOMIZE_ONEPLAYER_SWAPPING = false;
                GlobalEV.DISABLE_ONEPLAYER_SWAPPING = false;
                GlobalEV.UNLOCK_ALL_EQUIPMENT = false;
                GlobalEV.UNLOCK_ALL_LEVELS = false;
                GlobalEV.ENABLE_DROP_IN = false;
                GlobalEV.UNLOCK_MASTERIES = false;
                GlobalEV.UNLOCK_ALL_ITEMS = false;
                GlobalEV.DISABLE_CRASH_LOGS = false;
                GlobalEV.ENABLE_FMOD_PROFILING = false;
                GlobalEV.UNLOCK_ALL_MONOLITHS = false;
                GlobalEV.DISABLE_WARMONGER_DIALOGUE_TEST = false;
                GlobalEV.DISABLE_BLITWORKS_SPLASH = false;
                GlobalEV.SET_PREBUILD_PLAYER_LEVEL = 0;
                GlobalEV.IS_LQA = false;
                GlobalEV.DISABLE_BATCHING_GAUSSIAN_BLUR = false;
                GlobalEV.TRACE_SAVES = false;
            }

            if (GlobalEV.CREATE_CONSOLE_QA_VERSION)
                GlobalEV.DISABLE_CRASH_LOGS = true;
            if (GlobalEV.GENERATE_CRASH_DUMPS)
                Process.Start("cmd", "/C procdump.exe /accepteula -ma -e -w Brawler2D.exe %HOMEPATH%\\Documents\\");
            if (GlobalEV.ONLINE_ARCADE_MODE)
            {
                GlobalEV.ENABLE_DROP_IN = true;
                GlobalEV.DISABLE_ONEPLAYER_SWAPPING = true;
            }

            if (!BlitNet.DRM)
            {
                SDL.SDL_ShowSimpleMessageBox(SDL.SDL_MessageBoxFlags.SDL_MESSAGEBOX_ERROR, "Steamworks failed!", "Please load Full Metal Furies from the Steam client", IntPtr.Zero);
                return;
            }

            SteamManager instance = SteamManager.Instance;

            // this.Game.m_gameArgs = args;
            if (GlobalEV.DISABLE_CRASH_LOGS && !GameController.runningFromEditor)
            {
                // gameController.Run();
                // this.Game.Run();
                return;
            ...
            try
            {
             // using GameController gameController = new(args);
            // gameController.Run();
                ...
                // Brawler2D.GameRunner.instance = this.Game;
                this.Game.Run();
            }
            catch (Exception ex)
            {
                try
                {
                    string str1 = DateTime.Now.ToString("dd_MM_yyyy__HH_mm_ss", (IFormatProvider) new CultureInfo("en-US", false)
                    {
                        NumberFormat =
                        {
                    ...
                    string str2 = Path.Combine(path1, "Cellar Door Games", "Full Metal Furies", "Crash Logs");
                    if (!Directory.Exists(str2))
                        Directory.CreateDirectory(str2);
                    using StreamWriter streamWriter = new StreamWriter(Path.Combine(str2, "CrashLog_" + str1 + ".txt"), false);
                    streamWriter.WriteLine("Crash log generated in v" + (object) (byte) 1 + "." + (object) (byte) 2 + "." + (object) (byte) 2);
                    streamWriter.WriteLine(ex.ToString());
                }
                catch
                {
                    SDL.SDL_ShowSimpleMessageBox(SDL.SDL_MessageBoxFlags.SDL_MESSAGEBOX_ERROR, "SAVE THIS MESSAGE!", (!GlobalEV.CREATE_CHINESE_BUILD ? "Your game has encountered an error and failed to generate a crash log.  Please take a screenshot of this game error and send it to support@cellardoorgames.com.\n\nERROR: " : "游戏遇到错误并且无法生成崩溃日志。  请截屏游戏错误并发送至：support@cellardoorgames.com。\n\n错误：") + ex.ToString(), IntPtr.Zero);
                    return;
                }

                SDL.SDL_ShowSimpleMessageBox(SDL.SDL_MessageBoxFlags.SDL_MESSAGEBOX_ERROR, "SAVE THIS MESSAGE!", (!GlobalEV.CREATE_CHINESE_BUILD ? "Your game has encountered an error and a crash log has been generated in the game save folder.  Please send the generated crash log to support@cellardoorgames.com.\n\nERROR: " : "游戏遇到错误并且在游戏保存文件夹中生成崩溃日志。  请将生成的崩溃日志发送至：support@cellardoorgames.com。\n\n错误：") + ex.ToString(), IntPtr.Zero);
            }
        }
