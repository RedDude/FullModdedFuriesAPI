...
// using Brawler2D;
// using Brawler2D.Network;
// using Brawler2D.SDKs;

// namespace FullModdedFuriesAPI.Framework
// {
//     /// <summary>FMODF's implementation of the game's core multiplayer logic.</summary>
//     /// <remarks>
//     /// FMODF syncs mod context to all players through the host as such:
...
//     ///
//     /// Once a farmhand/server stored a context, messages can be sent to that player through the FMODF APIs.
//     /// </remarks>
...
//         /// <param name="eventManager">Manages FMODF events.</param>
...
//                         this.Monitor.Log($"Received context for farmhand {message.FarmerID} running {(model != null ? $"FMODF {model.ApiVersion} with {model.Mods.Length} mods" : "vanilla")}.", LogLevel.Trace);
...
//                         // parse message
//                         RemoteContextModel model = this.ReadContext(message.Reader);
//                         this.Monitor.Log($"Received context for {(model?.IsHost == true ? "host" : "farmhand")} {message.FarmerID} running {(model != null ? $"FMODF {model.ApiVersion} with {model.Mods.Length} mods" : "vanilla")}.", LogLevel.Trace);
...
//             // get valid peers
//             var sendToPeers = this.Peers.Values.Where(p => p.HasFmodf).ToList();
...
//                     }
//                 }
//                 else if (this.HostPeer?.HasFmodf == true)
//                 {
//                     if (this.LogNetworkTraffic)
