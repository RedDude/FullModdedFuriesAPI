...
namespace FullModdedFuriesAPI.Framework.Reflection
{
    ...
    internal class InterfaceProxyFactory
    {
        ...
        *********/
        ...
        {
            AssemblyBuilder assemblyBuilder = AppDomain.CurrentDomain.DefineDynamicAssembly(new AssemblyName($"FullModdedFuriesAPI.Proxies, Version={this.GetType().Assembly.GetName().Version}, Culture=neutral"), AssemblyBuilderAccess.Run);
            this.ModuleBuilder = assemblyBuilder.DefineDynamicModule("FullModdedFuriesAPI.Proxies");
        }
            ...
            where TInterface : class
        {
            lock (this.Builders)
            {
                ...
                string proxyTypeName = $"FullModdedFuriesAPI.Proxies.From<{sourceModID}_{typeof(TInterface).FullName}>_To<{targetModID}_{targetType.FullName}>";
