        ...
        public ModFolder ReadFolder(DirectoryInfo root, DirectoryInfo searchFolder)
        {
            ...
            if (manifestFile == null)
            {
                ...
                if (this.IsXnbMod(relevantFiles))
                    return new ModFolder(root, searchFolder, ModType.Xnb, null, ModParseError.XnbMod, "it's not a FMODF mod (see https://fmodf.io/xnb for info).");

                // FMODF installer
                if (relevantFiles.Any(p => p.Name == "install on Linux.sh" || p.Name == "install on macOS.command" || p.Name == "install on Windows.bat"))
                    return new ModFolder(root, searchFolder, ModType.Invalid, null, ModParseError.ManifestMissing, "the FMODF installer isn't a mod (you can delete this folder after running the installer file).");
            ...
            ModType type;
            {
                ...
                bool isFmodf = !string.IsNullOrWhiteSpace(manifest?.EntryDll);

                if (isContentPack == isFmodf)
