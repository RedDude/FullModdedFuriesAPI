...
namespace FullModdedFuriesAPI.Web.Framework.LogParsing
{
    /// <summary>Parses FMODF log files.</summary>
    public class LogParser
    {
        /*********
        ** Fields
        *********/
        /// <summary>A regex pattern matching the start of a FMODF message.</summary>
        ...
        /// <summary>A regex pattern matching FMODF's initial platform info message.</summary>
        private readonly Regex InfoLinePattern = new Regex(@"^FMODF (?<apiVersion>.+) with Stardew Valley (?<gameVersion>.+) on (?<os>.+)", RegexOptions.Compiled | RegexOptions.IgnoreCase);

        /// <summary>A regex pattern matching FMODF's mod folder path line.</summary>
        private readonly Regex ModPathPattern = new Regex(@"^Mods go here: (?<path>.+)", RegexOptions.Compiled | RegexOptions.IgnoreCase);

        /// <summary>A regex pattern matching FMODF's log timestamp line.</summary>
        ...
        /// <summary>A regex pattern matching the start of FMODF's mod list.</summary>
        private readonly Regex ModListStartPattern = new Regex(@"^Loaded \d+ mods:$", RegexOptions.Compiled | RegexOptions.IgnoreCase);

        /// <summary>A regex pattern matching an entry in FMODF's mod list.</summary>
        ...
        /// <summary>A regex pattern matching the start of FMODF's content pack list.</summary>
        ...
        /// <summary>A regex pattern matching an entry in FMODF's content pack list.</summary>
        ...
        /// <summary>A regex pattern matching the start of FMODF's mod update list.</summary>
        ...
        /// <summary>A regex pattern matching an entry in FMODF's mod update list.</summary>
        ...
        /// <summary>A regex pattern matching FMODF's update line.</summary>
        private readonly Regex FmodfUpdatePattern = new Regex(@"^You can update FMODF to (?<version>[^\s]+): (?<link>.+)$", RegexOptions.Compiled | RegexOptions.IgnoreCase);


        /*********
        ** Public methods
        *********/
        /// <summary>Parse FMODF log text.</summary>
        /// <param name="logText">The FMODF log text.</param>
        public ParsedLog Parse(string logText)
        {
            try
            {
                ...
                // parse log messages
                LogModInfo fmodfMod = new LogModInfo { Name = "FMODF", Author = "Pathoschild", Description = "", Loaded = true };
                ...
                foreach (LogMessage message in log.Messages)
                {
                    ...
                    // collect FMODF metadata
                    if (message.Mod == "FMODF")
                    {
                        // update flags
                        ...
                        else if (message.Level == LogLevel.Alert && this.FmodfUpdatePattern.IsMatch(message.Text))
                        {
                            Match match = this.FmodfUpdatePattern.Match(message.Text);
                            string version = match.Groups["version"].Value;
                            string link = match.Groups["link"].Value;
                            fmodfMod.UpdateVersion = version;
                            fmodfMod.UpdateLink = link;
                        }
                        ...
                        else if (message.Level == LogLevel.Info && this.InfoLinePattern.IsMatch(message.Text))
                        {
                            ...
                            fmodfMod.Version = log.ApiVersion;
                        }
                ...
                log.Mods = new[] { gameMod, fmodfMod }.Concat(mods.Values.OrderBy(p => p.Name)).ToArray();
                return log;
            }
        ...
        *********/
        ...
        /// <summary>Split a FMODF log into individual log messages.</summary>
        /// <param name="logText">The FMODF log text.</param>
        ...
        private IEnumerable<LogMessage> GetMessages(string logText)
        {
            ...
            while (true)
            {
                ...
                {
                    if (!builder.Started)
                        throw new LogParseException("Found a log message with no FMODF metadata. Is this a FMODF log file?");
