...
namespace FullModdedFuriesAPI.Web.Framework.Clients.GitHub
{
    ...
    internal class GitHubClient : IGitHubClient
    {
        ...
        *********/
        ...
        /// <param name="repo">The repository key (like <c>Pathoschild/FMODF</c>).</param>
        ...
        public async Task<IModPage> GetModData(string id)
        {
            ...
            if (!id.Contains("/") || id.IndexOf("/", StringComparison.OrdinalIgnoreCase) != id.LastIndexOf("/", StringComparison.OrdinalIgnoreCase))
                return page.SetError(RemoteModStatus.DoesNotExist, $"The value '{id}' isn't a valid GitHub mod ID, must be a username and project name like 'Pathoschild/FMODF'.");
        ...
        *********/
        ...
        private void AssertKeyFormat(string repo)
        {
            if (repo == null || !repo.Contains("/") || repo.IndexOf("/", StringComparison.OrdinalIgnoreCase) != repo.LastIndexOf("/", StringComparison.OrdinalIgnoreCase))
                throw new ArgumentException($"The value '{repo}' isn't a valid GitHub repository key, must be a username and project name like 'Pathoschild/FMODF'.", nameof(repo));
        }
    }
}
