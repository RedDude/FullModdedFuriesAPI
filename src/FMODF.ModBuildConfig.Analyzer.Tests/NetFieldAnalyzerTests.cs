...
using FMODF.ModBuildConfig.Analyzer.Tests.Framework;
using FullModdedFuriesAPI.ModBuildConfig.Analyzer;

namespace FMODF.ModBuildConfig.Analyzer.Tests
{
    ...
    public class NetFieldAnalyzerTests : DiagnosticVerifier
    {
        ...
        *********/
        ...
        const string SampleProgram = @"
            ...
            using Brawler2D;
            using Netcode;
            using SObject = Brawler2D.Object;
        ...
        *********/
        ...
        public void AvoidImplicitNetFieldComparisons_RaisesDiagnostic(string codeText, int column, string expression, string fromType, string toType)
        {
            ...
            DiagnosticResult expected = new DiagnosticResult
            {
                ...
                Message = $"This implicitly converts '{expression}' from {fromType} to {toType}, but {fromType} has unintuitive implicit conversion rules. Consider comparing against the actual value instead to avoid bugs. See https://fmodf.io/package/avoid-implicit-net-field-cast for details.",
        ...
        public void AvoidNetFields_RaisesDiagnostic(string codeText, int column, string expression, string netType, string suggestedProperty)
        {
            ...
            {
                Id = "AvoidNetField",
                Message = $"'{expression}' is a {netType} field; consider using the {suggestedProperty} property instead. See https://fmodf.io/package/avoid-net-field for details.",
                Severity = DiagnosticSeverity.Warning,
                Locations = new[] { new DiagnosticResultLocation("Test0.cs", NetFieldAnalyzerTests.SampleCodeLine, NetFieldAnalyzerTests.SampleCodeColumn + column) }
            };
