...
namespace FullModdedFuriesAPI.Mods.SaveBackup
{
    ...
    public class ModEntry : Mod
    {
        ...
        *********/
        ...
        private readonly string BackupLabel = $"{DateTime.UtcNow:yyyy-MM-dd} - FMODF {Constants.ApiVersion}";
        ...
        *********/
        ...
        public override void Entry(IModHelper helper)
        {
            ...
            {
                // init backup folder
                var backupFolder = new DirectoryInfo(this.BackupFolder);
                backupFolder.Create();
        ...
        *********/
        ...
        {
            var savesDir = new DirectoryInfo(Constants.SavesPath);
            try
            {
                // get target path
                ...
                if (!this.RecursiveCopy(savesDir, fallbackDir, entry => this.MatchSaveFolders(savesDir, entry), copyRoot: true))
                {
                ...
                {
                    this.Monitor.Log($"Backed up to {fallbackDir.FullName}.");
                    return;
                }
        ...
        /// <remarks>Derived from the FMODF installer code.</remarks>
        ...
        private bool RecursiveCopy(FileSystemInfo source, DirectoryInfo targetFolder, Func<FileSystemInfo, bool> filter, bool copyRoot = true)
        {
            ...
            switch (source)
            {
                ...
                case DirectoryInfo sourceDir:
                    ...
                    string targetSubFolderString = targetSubfolder.ToString();
                    Directory.CreateDirectory(targetSubFolderString);

                    //Copy all the files & Replaces any files with the same name
                    foreach (string newPath in Directory.GetFiles(sourceDirString, "*.*",SearchOption.AllDirectories))
                    {
                        File.Copy(newPath, newPath.Replace(sourceDirString, targetSubFolderString), true);
                    }
