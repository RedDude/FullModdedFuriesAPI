...
namespace FullModdedFuriesAPI.Internal.Patching
{
        ...
        *********/
        ...
        public static Harmony Apply(string id, IMonitor monitor, params IPatcher[] patchers)
        {
            ...
            foreach (IPatcher patcher in patchers)
            {
                ...
                {
                    monitor.Log($"Couldn't apply runtime patch '{patcher.GetType().Name}' to the game. Some FMODF features may not work correctly. See log file for details.", LogLevel.Error);
